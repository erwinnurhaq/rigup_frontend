...
const UserTransaction = () => {
    ...
    const paidStatusRender = (status) => {
        switch(status){
            ...
            case 2:
                return 'Paid'
            default:
                return 'Fail, check email for further information'
        }
    }

    const deliveredStatusRender = (status) => {
        switch(status){
            case 0:
                return 'Waiting'
            case 1:
    ...
    const renderTransactionList = () => list && list.length>0 ? list.map((i,index)=>(
        <TableRow key={index}>
            ...
            <TableCell>{paidStatusRender(i.paidStatus)}</TableCell>
            <TableCell>{deliveredStatusRender(i.deliveredStatus)}</TableCell>
            <TableCell>
